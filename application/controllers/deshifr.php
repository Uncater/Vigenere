...
class Deshifr extends CI_Controller {
    ...
    function nod($a, $b) {
        for ($i = $a; $i > 1; --$i) { //начинаю с числа на 1 меньше чем заданное
            if (($a % $i == 0) AND ( $b % $i == 0)) { // если оба числа делятся без остатка
                $mass = $i; //тут был массив для того чтобы возвращаять не первое, а все
                return $mass;
            }
        }
        if (isset($mass)) {
    ...
    public function index() {
        ...
        //$text = "kccgv oy ppiwusc yspt grwmpv tajwii an hivem asxawpp kg ush maatrèiw wxtywl pw nwfa";
        $text = "WRXIZQSXLFKUTWKQZIQWPDDGCLDFTQVTYGYSTVKHLSSYLOYKRUOFEEBNEDSSWRXIZQSXLFKUTWKQZIQWPDDGCLDFTQVTYGYSTVKHLSSYLOYKRUOFEEBNEDSSWRXIZQSXLFKUTWKQZIQWPDDGCLDFTQVTYGYSTVKHLSSYLOYKRUOFEEBNEDSS";
       ...
       // $text      = "ncusfkv rav fvxkzr wlrr yds zsih. yz dlh zx rgl, dru meaiqmkict mrvv; eey tr xzrp oip, ays udd qsk hzz, hh arw r nefseh wvtkii. lv wefedi rn grsu e wmihru, ej borh r qrnthv, rru vs jsfh r haq, ej xyz grsu scy clxp oezw, rv rrp jtkii kfjd rpu gzoy, wsnr, fm brvfyxc, iq xyi xjog sch njroh. jsdz phsgpv gaxkyiu oo viv xyz aoxvvroirr zr ydm, eyk lv gew xyid gaxky, eey llxkpv cehhvh kcep; jfv yz wdw nmjz eqslky oo nrfa kcaw rfxydnj imii castvrvy oq xymj blrfv, jfm grsu, ek rhlgy wfhe siftcz dlh esk cayi klvdr imcp fa ldyxlkzr lr klv juwwvx; rid nrfazig wlrx jpck ej xyzsh afycy bh fcmey aqcnep, ce wlfyxct lx hyzoe dw nicg tkek xyzy vlfycy wumeocz us xyizm ebij me brlrj, ej cayi klv haoeuc zi lhwj ekordgkmmz frvdw. yds rae lvvrw pryxceg: eeh kcaw arw hpiwi vrfpgk jfv ydm. ki yeu io iyixyzr lrkiixoxvji ndtk wgmidtv, flx cdvhh ltfi tki kskvl dfjxzieqgv tidnfmgpv, zvhv rjkzrzeihj; vng mk arn aoarcj nalh fj ydm, wlrx yz kqin lfr tr ovig xhumjxdvs zicp, za aqc dee vllzv tfnshwjiu ohh oesngegkv. qrt tkek fv orxpp wrdd rj lw, rid dpc sw ps! dru wf, vs wmec kdm rfjiiqeg, kfh sgevw lw, vqeuc frv!";
        ...
        $countSimb = 3; // кол-во сиволов для сравнения
        $text      = str_replace(" ", "", $text); // убираем пробелы они не шифруются
//        $text      = str_replace(".", "", $text);
//        $text      = str_replace(",", "", $text);
//        $text      = str_replace("(", "", $text);
//        $text      = str_replace(")", "", $text);
//        $text      = str_replace("!", "", $text);
//        $text      = str_replace("-", "", $text); // другие знаки тоже по хорошему убрать все но что то точнее не получается
        $allText   = iconv_strlen($text); //смотрим длинну текста
        for ($i = 0; $i <= $allText - $countSimb; $i++) { // запускаем цикл в котором
            $str_tmp = substr($text, $i, $countSimb); // берем достроку начиная с 0 элемента, по тому кол-ву элементов что мы задали
            for ($j = $i + 1; $j <= $allText - $countSimb; $j++) { // сравниваем с ней все остальные подстроки по столько же символов
                $str_tmp2 = substr($text, $j, $countSimb);
                if ($str_tmp == $str_tmp2) { // И если они совпадают
                    $arr[$str_tmp][] = $j - $i; // Сохраняем результат и расстояние между ними
                }
            }
        } // тут всё ок работает
        $max = 0; // далее я решил что чем больше совпадений - тем лушче ( но хз)
        foreach($arr as $key => $val ){//беру массив и ищу где больше всего было одинаковых подстрок
           $z = count($val);
            if ($z > $max) {
                $max = $z;
                $chars = $key;
        ...
        $elems = count($arr[$chars]);// беру этот массив
        for($i=0; $i < $elems +1; $i++){ // беру первый элемент
            for($j = $i +1; $j < $elems; $j++){ //и сравниваю с каждым последующим и
             //   echo $arr[$chars][$i] . ' and ' . $arr[$chars][$j].'<br/>'; // тут можно посмотреть что имелось ввиду
                $nod_tmp = $this->nod($arr[$chars][$i], $arr[$chars][$j] );// нахожу наибольший множитель (хотя может стоит брать не один а все
                if ($nod_tmp != null) { //бывает ловлю нулы - избавляюсь
                    $nods[] = $nod_tmp;
                }
            }
